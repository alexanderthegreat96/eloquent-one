 ...
 */
namespace LexSystems\Framework\Kernel\Helpers\Debugger;
class Debugger
{
    ...
    public static function renderLegend()
    {
        $config = self::DebugConfig();

        if($config)
        {
            $returns = '<small><b>Legend</b>: ';
            foreach($config as $key=>$val)
            {
               if($key != 'bgColor')
               {
                   $returns .= '<span style="color:'.$config->$key.';">'.ucfirst($key).'</span> ';
               }
            }
            $returns .= '</small>';
            return $returns;
    ...
    public static function var_dump($var, $return = false, $expandLevel = 1)
    {
        ...
        $html = '
            <pre style="margin-bottom: 18px;' .
            'background: '.self::DebugConfig()->bgColor.';'.
            ...
            'color: '.self::DebugConfig()->textColor.';' .
            'font-family: Menlo,Monaco,Consolas,\'Courier New\',monospace;">
            <h2>Debugger Reloaded</h2>'.self::renderLegend().'
...
';
    ...
    public static function var_dump_plain($var, $expLevel, $depth = 0, $done = array())
    {
        ...
        if (is_bool($var)) {
            $html .= '<span style="color:'.self::DebugConfig()->booleanColor.';">bool</span><span style="color:'.self::DebugConfig()->textColor.';">(</span><strong>' . (($var) ? 'true' : 'false') . '</strong><span style="color:'.self::DebugConfig()->textColor.';">)</span>';
        } elseif (is_int($var)) {
            $html .= '<span style="color:'.self::DebugConfig()->intColor.';">int</span><span style="color:'.self::DebugConfig()->textColor.';">(</span><strong>' . $var . '</strong><span style="color:'.self::DebugConfig()->textColor.';">)</span>';
        } elseif (is_float($var)) {
            $html .= '<span style="color:'.self::DebugConfig()->floatColor.';">float</span><span style="color:'.self::DebugConfig()->textColor.';">(</span><strong>' . $var . '</strong><span style="color:'.self::DebugConfig()->textColor.';">)</span>';
        } elseif (is_string($var)) {
            $html .= '<span style="color:'.self::DebugConfig()->stringColor.';">string</span><span style="color:'.self::DebugConfig()->textColor.';">(</span>' . strlen($var) . '<span style="color:'.self::DebugConfig()->textColor.';">)</span> <strong>"' . self::htmlentities($var) . '"</strong>';
        } elseif (is_null($var)) {
            $html .= '<span style="color:'.self::DebugConfig()->nullColor.';"><strong>NULL</strong></span>';
        } elseif (is_resource($var)) {
            $html .= '<span style="color:'.self::DebugConfig()->resourceColor.';">resource</span>("' . get_resource_type($var) . '") <strong>"' . $var . '"</strong>';
        } elseif (is_array($var)) {
            // Check for recursion
            if ($depth > 0) {
                foreach ($done as $prev) {
                    if ($prev === $var) {
                        $html .= '<span style="color:'.self::DebugConfig()->arrayColor.';">array</span>(' . count($var) . ') *RECURSION DETECTED*';
                        return $html;
                    }
                }
            ...
            $html .= (!empty($var) ? ' <img id="' . $uuid . '" src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs%3D" onclick="javascript:colToggle(this.id);" /><script>setImg("' . $uuid . '",' . $setImg . ',1);</script>' : '') . '<span style="color:'.self::DebugConfig()->arrayColor.';">array</span>(' . count($var) . ')';
            if (!empty($var)) {
            ...
            foreach ($done as $prev) {
                if ($prev === $var) {
                    $html .= '<span style="color:'.self::DebugConfig()->objectColor.';">object</span>(' . get_class($var) . ') *RECURSION DETECTED*';
                    return $html;
                }
            }
            // Keep track of variables we have already processed to detect recursion
            ...
            self::$hasArray = true;
            $uuid = 'include-php-' . uniqid() . mt_rand(1, 1000000);
            $html .= ' <img id="' . $uuid . '" src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs%3D" onclick="javascript:colToggle(this.id);" /><script>setImg("' . $uuid . '",' . $setImg . ',1);</script><span style="color:'.self::DebugConfig()->objectColor.';">object</span>(' . get_class($var) . ') <span id="' . $uuid . '-collapsable" style="' . $setStyle . '"><br />[<br />';
            $varArray = (array)$var;
            $indent = 4;
            $longest_key = 0;
